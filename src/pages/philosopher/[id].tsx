...
const Philosopher: React.FC<Props> = ({}) => {
    ...
    const philosopher = testPhilosophers.filter(
        (phil) => phil.id === parseInt(id)
    )[0];

    return (
        <main className='bg-redflowers min-h-screen w-full bg-fixed bg-cover bg-fade grid place-items-center py-20'>
            {philosopher ? (
                <section className='bg-stone-950 bg-opacity-60 w-11/12 sm:w-10/12 xl:w-3/4 min-h-[6rem] py-8'>
                    ...
                    <div className='py-16'>
                        <iframe
                            className='mx-auto'
                            title='Youtube Video'
                            src={`https://www.youtube.com/embed/${philosopher.videoUri}`}
                            width={560}
                            height={315}
                            frameBorder={0}
                        />
                    ...
                    <div className='grid place-items-center'>
                        ...
                        <p className='max-w-2xl text-xl'>{philosopher.about}</p>
                    </div>
