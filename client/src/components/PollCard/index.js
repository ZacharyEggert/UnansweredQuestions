                ...
                (    
                    <div class="w-11/12 flex-initial flex-row h-12 my-4 flex" id="poll-response" >
                        <div class="flex-1 bg-blue-600 flex align-middle content-center items-center text-center yes-btn" >
                            <p class="flex-1" onClick={() => {clickHandler(true)}}>YES</p>
                        </div>
                        <div class="flex-1 bg-red-600 flex align-middle content-center items-center text-center no-btn">
                            <p class="flex-1" onClick={() => {clickHandler(false)}}>NO</p>
                        </div>
                    </div>
                ):(    
                    <div class="w-11/12 flex-initial flex-col h-12 my-4" id="poll-results">
                        <div class="flex-1 flex content-start py-1 px-2 w-full">
                            <div class="bg-blue-600 overflow-hidden whitespace-nowrap" style={{ width: percent(poll.vote_yes, poll.vote_no) + `%` }} >
                                {percent(poll.vote_yes, poll.vote_no) + `%`} YES
                            </div>
                        </div>
                        <div class="flex-1 flex content-start py-1 px-2 w-full">
                            <div class="bg-red-600 overflow-hidden whitespace-nowrap" style={{ width: percent(poll.vote_no, poll.vote_yes) + `%` }} >
